...
Every successful mutation establishes a new history epoch. Reverse Microstep and Reverse Instruction cannot cross it, including a confirmed runtime program-word override. A deterministic invalid-word execution after that override may itself be reversed to Ready, but the edited word remains and the mutation boundary is still the history floor.
