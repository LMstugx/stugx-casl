# Phase 12D-VISUAL Circuit Visual Audit

## 1. Why This Audit Was Needed

Manual QA after `v1.0-rc7` found that the Circuit SVG still had systemic visual issues: floating marker dots, address/data route ambiguity near Memory, arrows that could appear detached from anchors, and too much animated activity on non-data guide paths. The follow-up Phase 12D-VISUAL2 pass removed active terminal arrows entirely and tightened dangling-wire checks. Phase 12D-VISUAL3 then removed default inactive guide wires and bounded Inspector Memory table height.

This pass is visual-only. It does not change CASL execution, emitted CASL, C++ lowering, the VM, the assembler, WASM, or mock core behavior.

## 2. Defect Categories

- Markers: junction and terminal dots could look like orphan nodes rather than meaningful circuit joins.
- Arrows: active terminal arrows could look detached or float on short segments, so v1.0 now uses arrowless active wires.
- Memory routing: Memory target addressing and Memory data movement were visually mixed.
- EAU routing: BASE, INDEX, and EA lanes must preserve label clearance.
- ALU / GR / MDR: LD and ST must remain ALU-bypass paths while compute instructions use ALU routes.
- Stack / CALL / RET: stack paths must show real return-address or stack data movement without fake stack-frame locals.
- Animation: active wire animation should be light, active-only, and frozen in visual review.
- Scroll: Focus Mode must keep the natural page-scroll model and avoid nested tiny scroll traps, while large data tables such as Inspector Memory use bounded internal scrolling.

## 3. Fixed Rules

- Junction dots are disabled by default. They may only be rendered for explicit semantic branch or merge points.
- Memory has no circular terminal marker, floating junction, or Memory-side guide circle in v1.0.
- `mar-to-memory` remains in path metadata as a target-highlight relation, but it is not rendered as a visible active blue address wire.
- Memory address targeting is shown by MAR activity, the Memory target badge, and the highlighted Memory row.
- Visible active Memory wires are data-flow routes: Memory row to MDR, MDR to Memory row, MDR to ALU, GR to MDR, and GR to ALU.
- Active terminal arrows, short terminal overlays, and floating arrowheads are not rendered in v1.0.
- The default v1.0 Circuit SVG renders active-flow wires only. Inactive guides, semantic-only paths, target-highlight templates, stale route templates, and background route lines are not drawn.
- Direction is shown by active color/weight, endpoint anchors, module and row highlights, Trace, Source Mapping, and Signal Probe.
- Inspector Memory uses a bounded internal table scroller for 64+ rows and must not stretch the main Circuit panel or the whole page.
- `visual-review-static` and reduced-motion mode freeze active wire animation.
- Visual review now includes `1280x720`, `1440x900`, and `1920x1080` viewport coverage.

## 4. Known Acceptable Limitations

- Full route debug overlays are deferred future work and are not part of the v1.0 release UI.
- The circuit still represents instruction-level data/control flow, not a true micro-cycle hardware simulator.
- FramePlan stack-frame information remains design preview only and does not create live stack-frame slot paths.

## 5. Validation Commands

Required validation for this phase:

```powershell
pnpm test
pnpm build
pnpm test:e2e
pnpm visual:review
pnpm visual:capture
powershell -ExecutionPolicy Bypass -File scripts/validate-all.ps1
powershell -ExecutionPolicy Bypass -File scripts/stress-check.ps1
pnpm audit
```

Visual artifacts generated by `pnpm visual:capture` must not be committed.

## 6. Result

The intended result is a stricter Circuit Focus visual baseline: Memory no longer shows circular markers or active address wires, active arrows are removed entirely, inactive guide routes are hidden by default, Inspector Memory scrolling is bounded, and visual review captures the main release-candidate viewports.
