...
- Related: [COMET Microcycle Mode](comet-microcycle-mode.md), [Reverse Instruction](reverse-instruction.md), [Known Limitations](known-limitations.md)
...
History holds at most 1000 microcycles in the current session. Earlier entries may become unavailable. Reverse Instruction can group retained entries back to the latest machine instruction's Fetch boundary. There is no Redo, Reverse Run, Reverse Macro, persistent history, or timeline scrubber.
